...
#[cfg(test)]
mod tests {
    use crate::protocol_params::constant_cost_models;

    #[test]
    fn preprod_conway_cost_models_match_current_protocol_view_shape() {
        let cost_models = constant_cost_models();

        let plutus_v1 = cost_models.inner.get(&0).expect("Plutus V1 cost model");
        let plutus_v2 = cost_models.inner.get(&1).expect("Plutus V2 cost model");
        let plutus_v3 = cost_models.inner.get(&2).expect("Plutus V3 cost model");

        assert_eq!(plutus_v1.len(), 332);
        assert_eq!(plutus_v2.len(), 332);
        assert_eq!(plutus_v3.len(), 350);
        assert_eq!(&plutus_v1[59..63], &[30623, 28755, 75, 1]);
        assert_eq!(&plutus_v3[50..57], &[123203, 7305, -900, 1716, 960, 57, 85848]);
        assert_eq!(&plutus_v3[342..350], &[95933, 1, 1, 11, 1000, 277577, 12, 21]);
    }
}
