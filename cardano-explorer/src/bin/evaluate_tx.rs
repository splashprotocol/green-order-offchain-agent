...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let key_path = args.next().expect("usage: evaluate_tx <maestro-key-path> <tx-cbor-path>");
    let tx_path = args.next().expect("usage: evaluate_tx <maestro-key-path> <tx-cbor-path>");
